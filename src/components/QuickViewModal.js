...
export function renderQuickViewModal() {
  return `
    <div class="overlay" id="quick-view-overlay"></div><div class="quick-view-modal" id="quick-view-modal" role="dialog" aria-modal="true" aria-label="Product Quick View"><button class="quick-view-close" id="quick-view-close" aria-label="Close"><span class="material-symbols-outlined">close</span></button><div class="quick-view-content" id="quick-view-content"></div></div>`;
}
...
export async function openQuickView(productId) {
  ...
  if (content) {
    content.innerHTML = `
      <div class="quick-view-image">${product.image && !String(product.image).startsWith('data:')
      ...
      </div><div class="quick-view-details"><div class="label">${product.category || ''}</div><h2 class="heading-3">${product.title}</h2><div class="pdp-price">${formatPrice(product.price)}</div><div class="text-body">${product.description || ''}</div><div style="font-size:var(--fs-sm);color:var(--color-text-secondary);">${[product.material, product.size, product.pages ? product.pages + ' pages' : null].filter(Boolean).join(' · ')}
        </div><div style="margin-top:var(--space-2);"><label style="font-size:var(--fs-sm);font-weight:var(--fw-medium);color:var(--color-text-secondary);display:block;margin-bottom:var(--space-1);">Quantity</label><div class="qty-stepper"><button class="qty-step-btn" id="qv-qty-minus" aria-label="Decrease">âˆ’</button><input type="number" class="qty-step-input" id="qv-qty" value="${moq}" min="${moq}" step="1"><button class="qty-step-btn" id="qv-qty-plus" aria-label="Increase">+</button></div><div style="font-size:var(--fs-xs);color:var(--color-text-tertiary);margin-top:var(--space-1);">Min. order: ${moq} units</div></div><div style="display:flex;gap:var(--space-3);flex-wrap:wrap;margin-top:auto;width:100%;"><button class="btn btn--accent btn--lg${isInCart || soldOut ? ' btn--added' : ''}" style="width:100%;" id="qv-add-cart"${isInCart || soldOut ? ' disabled' : ''}>${soldOut ? 'Sold Out' : isInCart ? 'Added to Cart' : 'Add to Cart'}
          </button></div><a href="/${product.slug}" class="btn btn--ghost" style="text-align:center;" id="qv-view-details">View Full Details →</a></div>`;
  }
